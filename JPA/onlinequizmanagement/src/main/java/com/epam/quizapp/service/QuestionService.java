...
public class QuestionService implements Service<String, List<Question>> {
	...
	public Response<String, List<Question>> init(RequestData<String, String> requestData) {
		...
		if(action.equals("getQuestionList")) {
			List<Question> questionList = QuestionDao.getInstance().getQuestionList(quizId);
			...
			List<Question> questionList = QuestionDao.getInstance().deleteQuestion(quizId, questionId);
			responseData.setAttribute("data", questionList);
		}
			...
			List<Question> questionList = QuestionDao.getInstance().addQuestion(quizId, question);
			responseData.setAttribute("data", questionList);
		}
